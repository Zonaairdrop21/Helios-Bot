    ...
    def print_question(self):
        while True:
            try:
                ...
                if option in [1, 2, 3, 4, 5, 6, 7]:
                    option_type = (
                        "Claim HLS Faucet" if option == 1 else "Bridge HLS Funds" if option == 2 else "Delegate Random Validators" if option == 3 else "Claim Delegate Rewards" if option == 4 else "Vote Governance Proposal" if option == 5 else "Deploy Token Contract" if option == 6 else "Run All Features"
                    )
        ...
        while True:
            try:
                ...
                if choose in [1, 2, 3]:
                    proxy_type = (
                        "With Free Proxyscrape" if choose == 1 else "With Private" if choose == 2 else "Without"
                    )
